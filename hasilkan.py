...
def generate_object_names(keywords_file, num_objects=1000):
    ...
    if len(keywords) < 2:
        raise ValueError("Jumlah kata kunci harus minimal 2 untuk menghasilkan objek.")
...
generated_objects.to_csv('hasil.txt', index=False)

print(f"{num_objects_to_generate} Nama objek telah disimpan ke dalam hasil.txt")
